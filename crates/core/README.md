...
Crate root exports (see `src/lib.rs`):

```rust
pub fn load_site_bundle(site_dir: &Path) -> Result<(SiteBundle, Vec<ConfigWarning>), CoreError>;
pub fn validate_site_bundle(bundle: &SiteBundle, template_dir: &Path) -> Result<Vec<ConfigWarning>, CoreError>;
pub fn render_site_bundle(bundle: &SiteBundle, target_dir: &Path, template_dir: &Path) -> Result<Vec<ConfigWarning>, CoreError>;
pub fn discover_content_bundles(project_root: &Path) -> Result<Vec<String>, CoreError>;
pub fn resolve_site_dir(project_root: &Path, site_input: &str) -> Result<PathBuf, CoreError>;
pub fn resolve_project_root() -> Result<PathBuf, CoreError>;
```
...
Submodules (requirements §4 mapping): `config`, `bundles`, `strict`, `assets`, `routing`, `render`, `widgets`, `fs_util`, `serve`.
